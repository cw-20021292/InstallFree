...
// * Hal (HW) *********************************************************************************
    ...
    Drv_Flowmeter_Module_Initialize();      // 유량 센서 관련 초기화
    Drv_ReedSW_Module_Initialize();         // Reed SW 제어 관련 초기화
    Drv_RTC_Module_Initialize();            // RTC 관련 초기화
    Drv_TDS_Module_Initialize();            // TDS 관련 초기화
    Drv_LevelSensor_Module_Initialize();    // 수위 센서 관련 초기화
    /* 얼음 관련 */
    Drv_Input_IceTray_Module_Initialize();  // Ice Tray 감지 제어 관련 초기화

    // Output *********************************************************************************
    Drv_AC_Motor_Module_Initialize();       // AC 모터 관련 초기화
    Drv_BoostPump_Module_Initialize();      // 부스트 펌프 관련 초기화
    ...
    Drv_EW_Module_Initialize();             // EW 모듈 제어 관련 초기화
    Drv_LeakSensor_Module_Initialize();     // 누수 센서 전원 제어 관련 초기화
    Drv_Stepmotor_Module_Initialize();      // Stepmotor 제어 관련 초기화
    Drv_StirringMoter_Module_Initialize();  // 교반 모터 제어 관련 초기화
    Drv_ColdTH_Module_Initialize();         // 냉수 온도 센서 전원 제어 관련 초기화
    Drv_UV_Module_Initialize();             // UV 제어 관련 초기화
    Drv_Voice_Module_Initialize();          // Voice IC 초기화
    Drv_SmallValve_Module_Initialize();     // 소형 밸브 제어 관련 초기화
    Drv_Valve_Module_Initialize();          // 일반 밸브 제어 관련 초기화
    /* 히팅 관련 */
    Drv_Heater_Module_Initialize();         // 일반 히터 관련 초기화
    Drv_Inst_Heater_Module_Initialize();    // 순간온수 히터 관련 초기화
    /* 냉각 관련 */
    Drv_BLDC_Comp_Module_Initialize();      // BLDC Comp 제어 관련 초기화
    Drv_Comp_Module_Initialize();           // 정속형 Comp 제어 관련 초기화
    Drv_TEM_Module_Initialize();            // TEM 제어 관련 초기화
    /* 얼음 관련 */
    Drv_SwingBar_Module_Initialize();       // Swing Bar 제어 관련 초기화
    Drv_AC_Motor_IceMaking_Module_Initialize();     // 제빙 관련 AC 모터들 관련 초기화
    Drv_GasSwitch_Module_Initialize();      // Gas Switch 제어 관련 초기화
    Drv_IceDoor_Module_Initialize();        // Ice Door 제어 관련 초기화
    Drv_IceTray_Module_Initialize();        // Ice Tray 제어 관련 초기화
...
// * Lib (Function) ***************************************************************************
    ...
    Lib_Water_Debug_Module_Initialize();    // 정수기 디버깅 제어 관련 초기화
    /* 히팅 관련 */
